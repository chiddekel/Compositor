// Portable port of Compositor/Rendering/CanvasViewport.swift (file-map tier:
// "Keep geometry; remove CoreGraphics import/use portable value types"). Logic is
// unchanged from the macOS original; only the Apple API surface is exchanged:
// `import CoreGraphics` is dropped because CGPoint/CGSize/CGRect/CGFloat are
// already provided by Foundation on Linux. SOLID: the viewport's geometry contract
// (document/view/zoom mapping) is preserved verbatim; the macOS original stays the
// source of truth.

import Foundation

/// Document: pixels with top-left origin. View: display points. Zoom 1 means actual display pixels.
