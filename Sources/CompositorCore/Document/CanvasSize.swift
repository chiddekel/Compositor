// Portable port of Compositor/Document/CanvasSize.swift (file-map tier: "Keep
// logic; replace Apple operations"). Logic is unchanged from the macOS original;
// only the Apple API surface is exchanged: `import CoreGraphics` is dropped because
// CGPoint/CGFloat are already provided by Foundation on Linux (and by the
// portable geometry glue when needed). SOLID: the value types keep their
// responsibilities and contracts (OCP/SRP); the macOS original stays the source of
// truth.
