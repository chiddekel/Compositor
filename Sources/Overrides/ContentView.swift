// Override, not a symlink — user-authorized, one-time exception to "compile Compositor/UI unmodified" after
// exhausting every non-upstream-editing option (see the SwiftUI-compat-runtime plan's `.onChange` root-cause
// writeup): `body`'s 11-call `.onChange` chain makes Swift's constraint solver time out, because two `.onChange`
// overloads (2-arg action, 0-arg action — both real, both needed elsewhere) must coexist, and no compiler flag or
// `@_disfavoredOverload` hint prevents the resulting combinatorial exploration for a chain this long. Confirmed by
// direct test: removing one overload entirely fixes it in 25s; disfavoring it does not, even combined with every
// solver-tuning flag `swift-frontend -help-hidden` lists.
//
// This file is a byte-for-byte copy of Compositor/ContentView.swift with exactly ONE change: `body` is split into
// `bodyCore` + `body`, the same technique upstream's own authors already used twice in this exact file
// (`toolHeaders`/`editorStack`/`editorChrome`) for the identical reason — their own comments say so directly
...
/// the actual app-lifecycle role on this platform, same principle as every other Qt-vs-AppKit boundary in this
/// port). `ContentView.swift` only ever reads three plain, portable members off it (`workspace`, `projects`,
/// `showEditor`), never anything app-lifecycle-shaped — so this carries exactly those three, faithfully, and
/// nothing else. `applicationDelegate` stays `nil` at every current call site (`ContentView` isn't wired into an
...
struct ContentView: View {
    ...
    // Split from `body` (the ONE deviation from upstream in this file): the same "chain outgrew the type checker"
    // pattern the two splits above already document, one level deeper. `bodyCore` carries the first four
    // `.onChange` calls (through the one whose closure body itself is thorniest, `session.document?.layers`);
    // `body` chains the rest onto it exactly as it chains everything onto `editorChrome` above. Purely a
    // compile-time restructuring — the resolved view tree and all behavior are unchanged.
    @ViewBuilder private var bodyCore: some View {
        editorChrome
