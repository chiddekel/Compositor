...
| `EditorSession.swift`/`LayerTransform.swift` transform editing | `Document/TransformEditLinux.swift` | `beginTransform` (preflight + undo snapshot)/`previewTransform` (history-suppressed)/`commitTransform` (one-step undo)/`cancelTransform`, `isMaskSelected` switches the target to the layer mask, mask-alone and group-member cases; `TransformEditTests`. Distortion is ported: `beginDistort`/`previewCorners` (twisted or collapsed shapes refused)/`commitDistort` (one undo step, layers + linked/placed masks warped with the carried placement), pinned by the 12 `DistortWarp` raster tests in `CropWandDistortTests`. Duplicate-transform, floating-selection merge, and the interactive preview cache remain (raster milestone). |
