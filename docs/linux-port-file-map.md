...
**Baseline:** a19db90, 2026-09-19

Derived from code graph, tracked-source inventory, import audit and detailed inspection of critical boundaries. Disposition is planning classification, not claim every function ported or proven equivalent. `Keep` means reuse intent and implementation where compatible; indirect Apple dependencies must still be tested. Original macOS files remain supported. Targets describe responsibilities, not one-for-one Apple API emulation.
...
*Line counts use newline-delimited segments, including trailing empty segment where present. All 158 tracked Swift/C/header files listed: 109 application, 49 test. Application source ~17,751 such lines. Counts are scale indicators, not rewrite estimates.
...
Preliminary engineering ranges, not measured delivery promises. Human estimates assume one experienced graphics/desktop engineer; assisted ranges assume sustained agent work plus that engineer's review and access to macOS reference execution. Parallel work overlaps; totals are not sums.
...
Overall order of magnitude: 3–6 engineer-months for dependable full port, or ~5–10 calendar weeks closely supervised assisted work with continuous verification. Re-estimate after first working journey. Vulkan not dominant workstream. No runtime integration, pixel equivalence or release timing demonstrated yet.
