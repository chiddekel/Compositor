...
| `TiledLayerTests.swift` | Quartz 2D vs Skia non-antialiased polygon clip rasterization: `TiledLayerRenderer.drawReplacing` divides the canvas into complementary non-antialiased path clips (`outline.subtracting(device)` and `device`) when painting rotated layers (25.0°). Quartz 2D's scan-converter leaves zero seam, while Skia's non-antialiased polygon clipping has a 1-pixel overlap along diagonal edges, causing translucent pixels to double-blend (diff 64 vs tolerance 12). Similarly, `paintingAScaledDownLayerDoesNotShiftItsPixels` detects this 1-pixel seam outside the brush box. |
